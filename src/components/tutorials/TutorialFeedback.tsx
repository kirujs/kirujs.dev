import { useEffect } from "kaioken"
import { className as cls } from "kaioken/utils"
import { tutorialFeedback } from "./TutorialStepsContext"

export function TutorialFeedback() {
  const feedback = tutorialFeedback.value

  useEffect(() => {
    if (feedback.show) {
      // Auto-hide after 5 seconds
      const timer = setTimeout(() => {
        tutorialFeedback.value = { ...feedback, show: false }
      }, 5000)
      return () => clearTimeout(timer)
    }
  }, [feedback.show])

  if (!feedback.show) return null

  return (
    <div className="fixed top-4 right-4 z-50">
      <div
        className={cls(
          "p-4 rounded-lg shadow-lg border max-w-sm transform transition-all duration-300",
          "animate-in slide-in-from-top-2 fade-in-0",
          feedback.type === "success" &&
            "bg-green-900/90 border-green-500/30 text-green-100",
          feedback.type === "info" &&
            "bg-blue-900/90 border-blue-500/30 text-blue-100",
          feedback.type === "warning" &&
            "bg-orange-900/90 border-orange-500/30 text-orange-100"
        )}
      >
        <div className="flex items-start gap-3">
          {/* Icon */}
          <div className="flex-shrink-0">
            {feedback.type === "success" && (
              <svg
                className="w-5 h-5 text-green-400"
                fill="currentColor"
                viewBox="0 0 20 20"
              >
                <path
                  fill-rule="evenodd"
                  d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z"
                  clip-rule="evenodd"
                />
              </svg>
            )}
            {feedback.type === "info" && (
              <svg
                className="w-5 h-5 text-blue-400"
                fill="currentColor"
                viewBox="0 0 20 20"
              >
                <path
                  fill-rule="evenodd"
                  d="M18 10a8 8 0 11-16 0 8 8 0 0116 0zm-7-4a1 1 0 11-2 0 1 1 0 012 0zM9 9a1 1 0 000 2v3a1 1 0 001 1h1a1 1 0 100-2v-3a1 1 0 00-1-1H9z"
                  clip-rule="evenodd"
                />
              </svg>
            )}
            {feedback.type === "warning" && (
              <svg
                className="w-5 h-5 text-orange-400"
                fill="currentColor"
                viewBox="0 0 20 20"
              >
                <path
                  fill-rule="evenodd"
                  d="M8.257 3.099c.765-1.36 2.722-1.36 3.486 0l5.58 9.92c.75 1.334-.213 2.98-1.742 2.98H4.42c-1.53 0-2.493-1.646-1.743-2.98l5.58-9.92zM11 13a1 1 0 11-2 0 1 1 0 012 0zm-1-8a1 1 0 00-1 1v3a1 1 0 002 0V6a1 1 0 00-1-1z"
                  clip-rule="evenodd"
                />
              </svg>
            )}
          </div>

          {/* Content */}
          <div className="flex-1">
            <p className="text-sm font-medium">{feedback.message}</p>
          </div>

          {/* Close button */}
          <button
            className="flex-shrink-0 ml-2 text-gray-400 hover:text-white transition-colors"
            onclick={() => {
              tutorialFeedback.value = { ...feedback, show: false }
            }}
          >
            <svg className="w-4 h-4" fill="currentColor" viewBox="0 0 20 20">
              <path
                fill-rule="evenodd"
                d="M4.293 4.293a1 1 0 011.414 0L10 8.586l4.293-4.293a1 1 0 111.414 1.414L11.414 10l4.293 4.293a1 1 0 01-1.414 1.414L10 11.414l-4.293 4.293a1 1 0 01-1.414-1.414L8.586 10 4.293 5.707a1 1 0 010-1.414z"
                clip-rule="evenodd"
              />
            </svg>
          </button>
        </div>
      </div>
    </div>
  )
}
