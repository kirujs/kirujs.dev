...
<Head.Content>
  ...
  <meta name="keywords" content="signal, state, global state, computed, effect, two way binding, For, Derive" />
  <meta name="description" content="Learn about Signals in Kiru - reactive primitives for managing state with computed values, effects, two-way binding, and fine-grained reactivity in components." />
</Head.Content>
...
`signal`, `computed`, and `watch` can all be created locally in Kiru 
...
<CopyInnerText prefix={`import { useSignal, useWatch } from "kiru"\n\n`}>
```tsx
function App() {
  const userName = useSignal("bob")
...
<CopyInnerText importsOverride={`import { useSignal, useComputed, For } from "kiru"`}>
```tsx
...
<DocsSection id="derive-component" title="<Derive />">
