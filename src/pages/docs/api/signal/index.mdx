...
export const head = defineHeadContent({
  title: "Kiru - Signal",
  extraMeta: [
    { name: "keywords", content: "signal, state, global state, computed, effect, untrack, two way binding, resource, async state" },
  ],
  description: "Learn about Signals in Kiru - reactive primitives for managing state with computed values, resources, effects, two-way binding, and fine-grained reactivity in components.",
})
...
<DocsSection id="untrack" title="untrack">
...
For `RefObject` and callback patterns, see the <Link to="/docs/api/ref">`ref` docs</Link>.
</DocsSection>
...
Resources automatically track any signals that are observed **synchronously** 
during the fetcher function's execution. This applies whether or not you provide 
a source parameter.
