...
- **`props`**: an object that provides access to the latest props via a 'getter'. This is useful when you need to access props in event handlers or other callbacks without deriving them into signals.
...
**Important:** Do not destructure `props` from the setup object, as this breaks the getter behavior and captures the props at the time of destructuring:
