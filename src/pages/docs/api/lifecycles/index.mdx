...
import ExampleSetup from "./example.setup.md"
import ExampleSetupTS from "./example.setup.ts.md"
import ExampleID from "./example.id.md"
import ExampleIDTS from "./example.id.ts.md"
import ExampleProps from "./example.props.md"
import ExamplePropsTS from "./example.props.ts.md"
import ExamplePropsWarning from "./example.props-warning.md"
import ExamplePropsWarningTS from "./example.props-warning.ts.md"
...
export const head = defineHeadContent({
  title: "Kiru - Lifecycles",
  extraMeta: [
    { name: "keywords", content: "lifecycle, setup, onMount, onBeforeMount, onCleanup, onHmr" },
  ],
  description: "Learn how to use lifecycle primitives in Kiru — setup, onMount, onBeforeMount, onCleanup, and onHmr — to derive props, run side effects, and clean up resources in your components.",
})
...
{lang => lang === "ts" ? <ExampleSetupTS /> : <ExampleSetup />}
</Derive>
...
- **`props`**: a Proxy object that provides access to the latest props.
...
<Derive from={siteCodeLang}>
{lang => lang === "ts" ? <ExampleIDTS /> : <ExampleID />}
</Derive>

You can also use the `props` object to access the latest props in event handlers or callbacks without deriving them into signals:

<Derive from={siteCodeLang}>
{lang => lang === "ts" ? <ExamplePropsTS /> : <ExampleProps />}
</Derive>

<CalloutBlock variant="info" prose className="w-full max-w-full">
**Important:** Do not destructure values from the `props` object, as this breaks the Proxy behavior and captures the values at the time of destructuring:

<Derive from={siteCodeLang}>
{lang => lang === "ts" ? <ExamplePropsWarningTS /> : <ExamplePropsWarning />}
</Derive>
</CalloutBlock>
