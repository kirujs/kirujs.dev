import { defineHeadContent } from "kiru/router"
import ExampleCode from "./example.md"
import { CopyInnerText } from "$/components/CopyInnerText"

export const head = defineHeadContent({
  title: "Kiru - Lazy",
  extraMeta: [
    { name: "keywords", content: "lazy, component, code-splitting, dynamic loading" },
  ],
  description: "Learn how to use lazy component in Kiru for code-splitting and dynamic component loading, improving performance by deferring component loading until needed.",
})


# Lazy

`lazy` is a higher-order component that's used to dynamically load components, 
deferring the loading of a component 
until it's needed. This is particularly useful for code-splitting, improving the 
performance of your application by loading smaller chunks of code.

A component produced by `lazy` can receive a `fallback` prop, 
which is displayed while the promise is being resolved. 
It accepts a JSX.Element, typically used to show loading indicators.

### Example

<CopyInnerText>
<ExampleCode />
</CopyInnerText>
